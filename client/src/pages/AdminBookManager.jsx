...
export default function AdminBookManager() {
  ...
  const onChange = (e) => {
    ...
    setForm((f) => ({ ...f, [name]: name === "total_copies" ? Number(value) : value }));
  };
  ...
  const filtered = useMemo(() => {
    ...
    return books.filter((b) => (b.title || "").toLowerCase().includes(q) || (b.isbn || "").toLowerCase().includes(q));
  }, [books, search]);
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/60">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="relative flex h-16 items-center justify-center">
            ...
            {/* Center title */}
            <h1 className="text-lg font-semibold text-slate-900">Book Management</h1>
            {/* Right: Logout */}
        ...
        <div className="flex-1 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              ...
              />
              <button onClick={fetchAll} className="rounded-lg bg-slate-100 px-3 py-1.5 text-sm hover:bg-slate-200">Refresh</button>
              <button onClick={startCreate} className="rounded-lg bg-indigo-600 px-3 py-1.5 text-sm text-white hover:bg-indigo-700">New</button>
            </div>
          </div>
                ...
                {loading ? (
                  ...
                  filtered.map((b) => (
                    <tr key={b.book_id} className="border-t">
                      ...
                      <td className="px-3 py-2">{b.available_copies}/{b.total_copies}</td>
                      <td className="px-3 py-2">
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs ${
                          b.status === "AVAILABLE" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
                        }`}>{b.status}</span>
                      </td>
                      <td className="px-3 py-2 text-right">
                        <button onClick={() => startEdit(b)} className="mr-2 rounded-md bg-slate-100 px-2 py-1 text-xs hover:bg-slate-200">Edit</button>
                        <button onClick={() => onDelete(b.book_id)} className="rounded-md bg-rose-600 px-2 py-1 text-xs text-white hover:bg-rose-700">Delete</button>
                      </td>
                    </tr>
                  ))
                )}
          ...
          {error && <div className="mt-3 rounded-md border-l-4 border-rose-500 bg-rose-50 p-3 text-sm text-rose-700">{error}</div>}
        </div>
        ...
        <div className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <form onSubmit={onSubmit} className="mt-4 space-y-3">
            ...
            <details className="text-xs text-slate-600">
              <summary className="cursor-pointer select-none">Add new category</summary>
              <AddCategory onAdded={async () => { await fetchAll(); }} />
            </details>
            <TextField label="Publication Year" name="publication_year" value={form.publication_year} onChange={onChange} inputMode="numeric" pattern="^[0-9]{4}$" maxLength={4} placeholder="e.g., 2024" title="Enter a 4-digit year like 2024" />
            <TextField label="Publisher" name="publisher" value={form.publisher} onChange={onChange} />
            <TextField label="Language" name="language" value={form.language} onChange={onChange} />
            <label className="block text-sm">
              <span className="text-slate-700">Description</span>
              <textarea name="description" value={form.description} onChange={onChange} rows={3} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </label>
            <TextField label="Shelf Location" name="location_shelf" value={form.location_shelf} onChange={onChange} />
            <TextField label="Total Copies" name="total_copies" value={form.total_copies} onChange={onChange} inputMode="numeric" />
            {editingId && (
...
function AddCategory({ onAdded }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      await CategoriesAPI.create({ category_name: name.trim(), description: desc });
      setName("");
      setDesc("");
      onAdded && onAdded();
    } catch (e) {
  ...
  return (
    <form onSubmit={submit} className="mt-2 flex flex-col gap-2">
      <div className="flex gap-2">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Category name" className="flex-1 rounded-md border border-slate-300 px-2 py-1" />
        <button disabled={busy} className="rounded-md bg-slate-800 px-3 py-1 text-xs text-white disabled:opacity-50">Add</button>
      </div>
      <input value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="Description (optional)" className="rounded-md border border-slate-300 px-2 py-1" />
