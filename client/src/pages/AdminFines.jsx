...
export default function AdminFines() {
  ...
  const settle = async (fineId) => {
    setLoading(true);
    setError("");
    try {
      await FinesAPI.settle(fineId);
      await load();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  const waive = async (fineId) => {
    setLoading(true);
    setError("");
    try {
      await FinesAPI.waive(fineId);
      await load();
    } catch (e) {
      setError(e.message);
    } finally {
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="mt-1 w-48 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm"
              >
        ...
        <section className="mb-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
            <div>
              ...
              <input
                value={creating.user_id}
                onChange={(e) =>
                  setCreating((v) => ({ ...v, user_id: e.target.value }))
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setCreating((v) => ({ ...v, amount: e.target.value }))
                }
                className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
              />
            </div>
            <div>
              ...
              <select
                value={creating.fine_type}
                onChange={(e) =>
                  setCreating((v) => ({ ...v, fine_type: e.target.value }))
                }
            ...
            <div className="flex items-end">
              <button
                onClick={createFine}
                className="w-full rounded-md border border-blue-600 bg-blue-600 px-3 py-2 text-sm text-white hover:bg-blue-700"
              >
                Create
              </button>
            </div>
                    ...
                    <td className="px-3 py-2 text-right">
                      {f.status === "UNPAID" ? (
                        <div className="flex justify-end gap-2">
                          <button
                            onClick={() => settle(f.fine_id)}
                            className="rounded-md border border-green-600 bg-green-600 px-2 py-1 text-xs text-white hover:bg-green-700"
                          >
                            Settle
                          </button>
                          <button
                            onClick={() => waive(f.fine_id)}
                            className="rounded-md border border-amber-600 bg-amber-500 px-2 py-1 text-xs text-white hover:bg-amber-600"
                          >
                            Waive
                          </button>
                        </div>
                ...
                {fines.length === 0 && (
                  <tr>
                    <td
                      ...
                      colSpan={6}
                    >
                      {loading ? "Loading…" : "No fines found."}
                    </td>
                  </tr>
                )}
              </tbody>
