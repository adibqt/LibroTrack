...
const fetchJson = (path) => api.request(path);
  ...
  const load = async () => {
    ...
    try {
      const [p, a, f] = await Promise.all([
        fetchJson("/reports/popular-books"),
        fetchJson("/reports/member-activity"),
        fetchJson("/reports/fines"),
      ]);
      setPopular(p);
      setActivity(a);
      setFines(f);
              ...
              <div className="mt-3 h-40 w-full">
                {(() => {
                  const top5 = [...popular]
                    .sort(
                      (a, b) =>
                        (Number(b.loan_count) || 0) -
                        (Number(a.loan_count) || 0)
                    )
                    .slice(0, 5);
                  if (top5.length === 0)
                    return (
                ...
                {(() => {
                  const top5 = [...activity]
                    .sort(
                      (a, b) =>
                        (Number(b.loan_count) || 0) +
                        (Number(b.reservation_count) || 0) -
                        ((Number(a.loan_count) || 0) +
                          (Number(a.reservation_count) || 0))
                    )
                    .slice(0, 5);
                  if (top5.length === 0)
                    return (
                ...
                {activity
                  .sort(
                    (a, b) =>
                      b.loan_count +
                      b.reservation_count -
                      (a.loan_count + a.reservation_count)
                  )
                  .slice(0, 5)
                  .map((m) => (
                    <div
                    ...
                    >
                      ...
                      <span className="font-medium text-slate-900">
                        {m.loan_count + m.reservation_count}
                      </span>
                    </div>
                  ))}
                {activity.length === 0 && (
                ...
                {fines[0] ? (
                  <>
                        ...
                        <PieChart>
                          ...
                          <Pie
                            data={[
                              {
                                name: "Unpaid",
                                value: Number(fines[0].unpaid_fines) || 0,
                              },
                              {
                                name: "Paid",
                                value: Number(fines[0].paid_fines) || 0,
                              },
                              {
                                name: "Waived",
                                value: Number(fines[0].waived_fines) || 0,
                              },
                            ]}
                            dataKey="value"
                            nameKey="name"
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                      <div className="text-slate-600">Total fines</div>
                      <div className="text-right font-medium">
                        {fines[0].total_fines}
                      </div>
                      <div className="text-slate-600">Unpaid</div>
                      <div className="text-right font-medium">
                        {fines[0].unpaid_fines} ({fines[0].unpaid_amount})
                      </div>
                      <div className="text-slate-600">Paid</div>
                      <div className="text-right font-medium">
                        {fines[0].paid_fines} ({fines[0].paid_amount})
                      </div>
                      <div className="text-slate-600">Waived</div>
                      <div className="text-right font-medium">
                        {fines[0].waived_fines} ({fines[0].waived_amount})
                      </div>
                    </div>
                  </>
                ) : (
            ...
            <div className="mb-4 h-72 w-full">
              {(() => {
                const top = [...popular]
                  .sort(
                    (a, b) =>
                      (Number(b.loan_count) || 0) - (Number(a.loan_count) || 0)
                  )
                  .slice(0, 10);
                if (top.length === 0)
